...
/* Prepare runtime removal of <srv> server. Basically, this purges every
 * references to this instance from other objects, for example in the parent
 * proxy. One effect is to liberate the server name slot which can be reused
 * for a newly created instance. Active check and agent-check are scheduled for
 * deletion.
 *
 * Server is marked as deleted with an associated event published. The server
 * instance will be finally freed when its refcount reaches zero. In the
 * meantime, the server is virtually invisible in the process. The only
 * exception is the GUID tree which still references it until its final free.
 *
